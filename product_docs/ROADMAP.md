...
**体验关键指标 (Experience KPI)**：

- **首屏可用时间**：用户点击“生成分析报告”后，20 秒内看到“可用核心结论”（P50），45 秒内看到（P95）
- **完整报告交付时间**：90 秒内完成全部模块（P95），且最终输出不缩水
- **失败可恢复**：分析失败有明确原因与重试路径，不要求用户重新上传
...
#### 1.5 分析时长与多模型协作（V1.6+ - 关键成败项）
- [ ] **分段交付**：先交付“摘要 + Top 错因 + 最短改法 + 证据/置信度”，其余模块后台补齐
- [ ] **多模型分工**：Extract（视觉抽取）/ Diagnose（推理报告）/ Practice（出题与验收）/ Merge（整合校验）
- [ ] **并行与分块**：多页图片按页并行抽取，Reduce 合并去重
- [ ] **结构化稳定性**：降低“JSON 修复二次调用”比例，提升 P95/P99
- [ ] **缓存与去重**：重复分析/重试/切换模型时复用抽取结果，显著缩短体验时长
