...
【关键输出规范 - 必须严格遵守】：
1. 使用标准中文字符，严禁使用 LaTeX 代码（如 $x^2$、\\frac{}{}）或 Markdown 代码块标记（如 \`\`\`json）
2. 所有数学公式必须用文字描述或 Unicode 符号表示（如：x² 而非 $x^2$，√2 而非 \\sqrt{2}，≤ 而非 \\leq）
3. 基于图片内容进行分析，不得编造题号、得分或错因，证据必须来自试卷原文
4. 每个分析结论必须包含【题号】【得分】【证据】【置信度】【最短改法】五要素，缺一不可
...
请输出为标准的 JSON 格式，不要包含 Markdown 代码块标记（\`\`\`json 或 \`\`\`），直接输出 JSON 对象。
所有文本内容使用标准中文，数学公式使用 Unicode 符号（如 x²、√、≤、≥、π）而非 LaTeX 代码。
...
不要包含 Markdown 代码块标记（\`\`\`json 或 \`\`\`），直接输出 JSON 字符串。
所有数学公式使用 Unicode 符号（如 x²、√、≤）而非 LaTeX 代码（如 $x^2$、\\sqrt{}）。
`;
...
【关键要求 - 必须严格遵守】：
1. 使用标准中文，严禁使用 LaTeX 代码或 Markdown 标记
2. 所有数学公式用 Unicode 符号（x²、√、≤）或文字描述
3. 基于图片内容分析，不得编造题号、得分或错因
4. 每个错因必须有明确的试卷证据支持
5. forStudent.problems 数组每条必须包含：【知识点】【题号】【得分】【错因】【证据】【置信度】【最短改法】七要素
6. 【得分】必须写成"本题得分/本题满分"格式，例如 1/4、0/2
7. 【证据】必须来自试卷原文，不得为空或模糊描述
8. 【置信度】必须真实反映识别准确性（高/中/低）
9. 所有输出内容必须清晰可读，不得包含乱码或不可见字符
